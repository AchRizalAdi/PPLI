                        ...
                        <div class="profile-setting-panel-wrap">
                            <div class="table">
                                <table class="table mb-0 table-s2" id="dataTable">
                                    <thead class="fs-14">
                                        <tr>
                                            <th scope="col" v-for="(list, i) in SectionData.groupData.groupTableHead" :key="i">{{ list }}</th>
                                        </tr>
                                    </thead>
                                    <tbody class="fs-13">
                                        <tr v-for="(item) in roles.data"  :key="item.id">
                                            <td>{{item.name}}</td>
                                            <td class="row">
                                                <router-link  :to="{name: 'privilege', params: { id: item.id }}" class="col- p-0 m-0 icon-btn" title="Show"><em class="fa-solid fa-gears" ></em></router-link>
                                                ...
                                                <button  @click="showDelete(item.id)" class="col- p-0 m-0 icon-btn" title="Delete" ><em class="ni ni-trash"></em></button>
                                            </td>
                                        </tr> 
                                    </tbody>
                                </table>
...
export default {
  ...
  methods: {
    ...
    postRoles(){
            axios.post('http://127.0.0.1:8000/api/role/store', {
                name: this.name,
            }).then(response => {
            this.showPost();    
            this.getRoles()
        ...
        putRoles(id){
            axios.post('http://127.0.0.1:8000/api/role/update/'+id, {
                name: this.name,
            }).then(response => {
        ...
        showRoles (id){
            // alert(id);
            axios.get('http://127.0.0.1:8000/api/role/show/'+id)
            .then(function (response) {
                // alert(response);
        ...
        deleteRoles (id){
    // alert(id);
            axios.delete('http://127.0.0.1:8000/api/role/'+id)
            .then(function () {
