<template>
   <div class="col-lg-9 ps-xl-5">
                        ...
                        <div class="profile-setting-panel-wrap">
                            ...
                            <div class="form-floating mb-3">
                                <input type="password" class="form-control password" id="password" placeholder="Password" v-model="password" required>
                            ...
                            <select class='form-control' v-model="roles" required>
                                <option v-for='item in roless.data' :value='item.name' :key='item.id'>{{ item.name }}</option>
                            </select>
                             </div>
                             <router-link to="/pengaturan" type="button" class="btn btn-sm btn-dark " >Back</router-link>
                             <button class="btn btn-sm btn-dark w-20 ms-2" type="submit" >Create User</button>
                            </form>
  ...
  methods: {
    resetnama() {
        this.name=null;
         ...
         postUser(){
            axios.post('http://127.0.0.1:8000/api/admin/adduser', {
                name: this.name,
                email: this.email,
